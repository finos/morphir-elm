...
describe("MEP extension release", () => {
  ...
  test.each([
    ...
    expect(() => parseReleaseTag(tag, { ...extensionMetadata, version: "0.1.0" })).toThrow(message);
  });
  ...
  test("the targets are the six platforms the morphir CLI is released for", () => {
    ...
    for (const target of RELEASE_TARGETS) {
      ...
      expect(target.executable).toBe(windows ? "morphir-elm-extension.exe" : "morphir-elm-extension");
  ...
  test("asset names carry the version and the platform, like the morphir CLI assets", () => {
    const linux = RELEASE_TARGETS.find((target) => target.platform === "x86_64-unknown-linux-gnu")!;
    const windows = RELEASE_TARGETS.find((target) => target.platform === "aarch64-pc-windows-msvc")!;
    expect(assetName("0.1.0", linux)).toBe("morphir-elm-extension-0.1.0-x86_64-unknown-linux-gnu.tgz");
    expect(assetName("0.1.0", windows)).toBe("morphir-elm-extension-0.1.0-aarch64-pc-windows-msvc.zip");
  });

  test("a checksum file is one sha256sum line", () => {
    expect(checksumLine("ab".repeat(32), "asset.tgz")).toBe(`${"ab".repeat(32)}  asset.tgz\n`);
  });
  ...
  test("the descriptor describes a process extension with one artifact per platform", () => {
    const descriptor = releaseDescriptor(extensionMetadata, "a".repeat(40), [
      ...
      { platform: "aarch64-apple-darwin", artifact: "two.tgz", sha256: "1".repeat(64) },
    ]);
  ...
  test("a descriptor with a bad commit or digest is refused", () => {
    const artifact = { platform: "x86_64-unknown-linux-gnu", artifact: "one.tgz", sha256: "0".repeat(64) };
    expect(() => releaseDescriptor(extensionMetadata, "main", [artifact])).toThrow(/git commit/);
    expect(() =>
      releaseDescriptor(extensionMetadata, "a".repeat(40), [{ ...artifact, sha256: "nope" }]),
    ).toThrow(/sha256/);
    expect(() => releaseDescriptor(extensionMetadata, "a".repeat(40), [])).toThrow(/no artifacts/);
  });
});
